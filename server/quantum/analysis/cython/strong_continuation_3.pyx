        ...
        if wrb_hg[v1] and d[v1] == 1:
            ...
            for v2 in range(v1 + 4, v1 + 4 + contraction):
                if wrb_hg[v2] and d[v2] == 1:
                    ...
                    if v2 - v1 - 1 > bxc[v1]:
                        break
                    # V2 must have the bodies of the prior contracting
                    # volatility < body mid-point of V2 or V1 must have the
                    # bodies of the prior contracting volatility < body mid-
                    # point of V1
                    if bn.nanmax(h[v1+1:v2]) > bmp[v1] and bn.nanmax(h[v2+1:v2+3]) > bmp[v2]:
                        break
        ...
        # Bearish Strong Continuation
            ...
            for v2 in range(v1 + 4, v1 + 4 + contraction):
                if wrb_hg[v2] and d[v2] == -1:
                    ...
                    # bodies of the prior contracting volatility > body mid-
                    # point of V1
                    if bn.nanmin(l[v1+1:v2]) > bmp[v1] and bn.nanmin(l[v2+1:v2+3]) > bmp[v2]:
                        break
                    #  Contraction volatiliti must share min 1 pip with V2
